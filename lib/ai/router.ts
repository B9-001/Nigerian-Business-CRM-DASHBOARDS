 ...
 * wired into the UI yet — this reads a safe default).
 */
export function routeModel(task: AITask): RouteResult {
  const primary = process.env.AI_DEFAULT_PROVIDER === 'google' ? googleAIProvider : openAIProvider

  switch (task) {
    case 'simple':
      return { provider: primary, model: primary.name === 'openai' ? 'gpt-4o-mini' : 'gemini-1.5-flash' }
    case 'complex':
      return { provider: primary, model: primary.name === 'openai' ? 'gpt-4o' : 'gemini-1.5-pro' }
    case 'research':
...
export function getFallbackProvider(): AIProvider {
  return process.env.AI_FALLBACK_PROVIDER === 'openai' ? openAIProvider : googleAIProvider
}
...
export function estimateCostNgn(model: string, inputTokens: number, outputTokens: number): number {
  const RATE_PER_1K_NGN: Record<string, { input: number; output: number }> = {
    ...
    'gemini-1.5-pro': { input: 1.6, output: 6.4 },
  }
